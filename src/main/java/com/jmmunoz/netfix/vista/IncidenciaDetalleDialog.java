...
public class IncidenciaDetalleDialog extends JDialog {
    ...
    private void initComponents() {
        ...
        sectClient = createSection(
                com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.client.title"), contClient);

        contContract = new JPanel();
        sectContract = createSection(
                com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.contract.title"),
                contContract);

        contInci = new JPanel();
        sectInci = createSection(
                com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.incid.title"), contInci);

        contComen = new JPanel();
        sectComen = createSection(
                com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.comments.title"), contComen);

        contDevices = new JPanel();
        sectDevices = createSection(
                com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.devices.title"),
                contDevices);
        ...
        try {
            ...
            if (rs != null && rs.next()) {
                ...
                addDetailRow(contClient,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.name"),
                        rs.getString("nombre_cliente"));
                addDetailRow(contClient,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.dni"),
                        rs.getString("dni_cliente"));

                // Llenar Contrato
                addDetailRow(contContract,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.contract.id"),
                        String.valueOf(idContrato));
                addDetailRow(contContract,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.plan"),
                        rs.getString("tipo_servicio"));

                // Llenar Incidencia
                addDetailRow(contInci,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.incid.id"),
                        String.valueOf(rs.getInt("id_incidencia")));
                String estado = rs.getString("estado");
                addDetailRow(contInci,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.status"),
                        (estado != null ? estado.toUpperCase() : "-"));
                addDetailRow(contInci,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.report.date"),
                        rs.getString("fecha_reporte"));
                addDetailRow(contInci,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.description"),
                        rs.getString("descripcion"));

                String tecnico = rs.getString("tecnico");
                addDetailRow(contInci,
                        com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.tech"),
                ...
                if (solucion != null && !solucion.isEmpty()) {
                    addDetailRow(contInci,
                            com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.label.solution"),
                            solucion);
                }
    ...
    private void cargarComentarios() {
        try {
            ...
            while (rs != null && rs.next()) {
                hasComen = true;
                String info = String.format("[%s] %s: %s", rs.getString("Fecha"), rs.getString("Agente"),
    ...
    private void cargarAparatos(int idContrato) {
        try {
            ...
            while (rs != null && rs.next()) {
                String desc = com.jmmunoz.netfix.config.AppConfig.getInstance().getMessage("incid.detail.device.format",
                        rs.getString("modelo"),
